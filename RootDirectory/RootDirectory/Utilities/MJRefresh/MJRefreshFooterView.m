	...
	switch (_state = state)
    {
        ...
        case RefreshStateRefreshing:
        {
            ...
            [UIView animateWithDuration:0.2 animations:^{
                // 1.顶部多出65的滚动范围
                UIEdgeInsets inset = _scrollView.contentInset;
                inset.bottom = self.frame.origin.y - _scrollView.contentSize.height +kViewHeight + self.bottomInset;
                _scrollView.contentInset = inset;
...
- (CGFloat)validY
{
    return MAX(_scrollView.contentSize.height, _scrollView.frame.size.height) - _scrollView.frame.size.height + self.bottomInset;
}
